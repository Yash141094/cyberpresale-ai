...
def fetch_competitor_market_intel(competitor_names_list, geo, industry, rfp_type):
    ...
    prompt = (
        ...
        "1. Annual IT services revenue in USD — ALWAYS include the full unit (billion or million). "
        "Example: '$22.2 billion' not '$22.2'. Also include total company revenue if known.\n"
        ...
        "{\n"
        "  \"CompetitorName\": {\n"
        "    \"revenue_usd\": \"$22.2 billion IT services revenue (FY2023); $29 billion total\",\n"
        ...
        "\"value\": \"$120 million\", \"year\": \"2023\", \"source\": \"press release\"}\n"
        "    ],\n"
...
def generate_competitive_with_competitors(rfp_text, competitor_names):
    ...
    prompt = (
        ...
        "IMPORTANT: Keep each cell under 8 words. Do NOT use the | character inside cells.\n"
        "| Competitor | Revenue (full unit) | Geo Strength | Threat Level | Strongest Card | Counter-Move |\n"
        "|------------|--------------------|--------------|--------------|-----------------|--------------|\n"
        "(one data row per competitor — keep all cells concise)\n\n"
        "## WHERE WE WIN\n"
